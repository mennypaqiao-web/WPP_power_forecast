...
st.write(
    """
    Это приложение прогнозирует мощность ветровой электростанции на основе прогноза погоды OpenWeatherMap.
    Введите API-ключ, загрузите таблицу мощности и получите прогноз общей генерации.
    """
)
...
def load_power_table_from_df(df_raw):
    ...
    power_table_df = power_table_df.sort_values(by="Скорость ветра (м/с)").reset_index(drop=True)
